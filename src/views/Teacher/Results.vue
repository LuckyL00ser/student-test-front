<template>
	<div>
		...
		<div class="my-3" v-if="!all && selectedSubjectId">
			<b-btn class="mr-2" @click="getExcel(selectedSubjectId)"
				>Pobierz wyniki w Excelu</b-btn
			>
			<b-btn class="mr-2" variant="success" @click="getPDF(selectedSubjectId)"
				>Pobierz wyniki w PDF</b-btn
			>
			<b-btn variant="info" @click="getDOCX(selectedSubjectId)"
				>Pobierz wyniki w DOCX</b-btn
			>
		</div>
		...
		filtered() {
			...
			if (this.all)
				list = this.results.filter(x =>
					this.teacherSubjects.includes(x.generateTest.test.subject.id),
				);
			else
				list = this.results.filter(
					x => this.selectedSubjectId == x.generateTest.test.subject.id,
				);

			if (this.onlyMarked === 'marked_only') list = list.filter(x => x.mark);
