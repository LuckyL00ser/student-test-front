<template>
	<b-card class="shadow overflow-hidden mb-3 ">
		<div class="d-flex flex-column h-100">
			...
			<p v-if="currentQuestion!=questions.length" class="d-flex justify-content-between align-items-center w-100">
				<span>Pytanie {{currentQuestion+1}}/{{questions.length}}</span>
				<span v-if="questions.length">Punkty za to pytanie: <b>{{ questions[currentQuestion].points }} </b></span>
			</p>
			<transition-group name="slide" mode="in-out" tag="div" class="h-100 d-flex">
					...
					<div class="h-100 d-flex flex-column">
						...
						<b-row class="flex-grow-1">
							...
							<b-col class="col-12 col-lg-6  h-100 pb-2">
								...
								<b-textarea class="h-100" v-else placeholder="Odpowiedź opisowa"></b-textarea>
							</b-col>
						</b-row>
					</div>
				</div>
